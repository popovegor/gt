...
namespace GT.Web.Site.WebServices.Ajax
{
    ...
    [ToolboxItem(false)]
    [ScriptService]
    [GenerateScriptType(typeof(MessageDirections))]
    ...
    public class MessageSystemService : BaseWebService
    {
        ...
        [WebMethod]
        [ScriptMethod]
        public string AddMessage(Guid recipient, string subject, string body)
        {
            string result = string.Empty;
            if (recipient != Guid.Empty
                && false == string.IsNullOrEmpty(subject)
                && false == string.IsNullOrEmpty(body))
            {
                User r = UsersFacade.GetUser(recipient);
                if (r != null)
                {
                    try
                    {
        ...
        public string[] GetRecipientList(string prefixText, int count)
        {
            ...
            foreach (User u in UsersFacade.GetAllUsers())
            {
